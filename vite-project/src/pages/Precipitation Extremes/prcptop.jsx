...
function PRCPTOP_PPT_Extreme() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const res = await axios.post('http://localhost:3000/index/prcptop_post', { location: selectedOption.value,  year: cred.year, index: data.index });
    console.log(res);
  ...
  const handlegetSubmit = async (e) => {
    ...
    const response = await axios.post("http://localhost:3000/index/prcptop_get", {
      year: cred.year,
    });
        ...
        <div className="bg-white p-5 md:p-8 rounded-lg shadow-md w-full ">
          ...
          <div className="containera vxyz flex flex-col md:flex-row justify-center items-center space-y-4 md:space-y-0 md:space-x-8 mt-4 md:mt-0">
            ...
            <div className="w-full md:w-auto">
              <div className="text-lg font-bold mb-4 flex justify-center "><h2 className="text-center heading_ssi">Annual total precipitation on wet days (PRCPTOP)</h2></div>
            ...
            <div className="flex flex-row container123">
              ...
              {user && (
                <>
                  ...
                  />
                  <h6 className="text-lg font-semibold mb-4">Enter PRCPTOP :</h6>
                  <input
              ...
              <div className="get_data_box bg-gray-100 rounded-md p-4">
                <h3 className="text-lg font-semibold mb-2">Get PRCPTOP</h3>
                <h6 className="text-lg font-semibold mb-2">Select Year :</h6>
                <select
                  className="entry w-full mb-3 p-2 border border-gray-300 rounded"
                  required
...
{colors.map((color, index) => (
  <div key={index} className="color-box-container">
    ...
    {index + 1 == 1 ? <> <span style={{fontSize:"12px"}} className="color-index">0 - 750</span></> : <></>}
    {index + 1 == 2 ? <> <span style={{fontSize:"12px"}} className="color-index">  750 - 1150</span></> : <></>}
    {index + 1 == 3 ? <> <span style={{fontSize:"12px"}} className="color-index">1150 - 2000</span></> : <></>}
    {index + 1 == 4 ? <> <span style={{fontSize:"12px"}} className="color-index">More than 2000</span></> : <></>}
  </div>
))}
</div>
            </div>
...
export default PRCPTOP_PPT_Extreme;